...
| MTE / `TagOp` | 🟡 | 🟡 | 🔴 | 🟡 | the RN-I sources `TagOp`, `TagGroupID`, `Tag` and `TU` from the AXI sideband and reports the `TagMatch` verdict on `BUSER` (see [below](#what-the-rn-i-generates)); the HN-I answers every read `TagOp = Invalid`, which section 12.11.3 (p.12-387, MUST) fixes for a Completer holding no Allocation Tag. the SN-F **stores** Allocation Tags over the AXI `W/RUSER` sideband — a `TagOp=Update` write installs the tags its TU bits name (section 12.5.2 p.12-379, MUST) and a `Transfer` or `Fetch` read is answered `TagOp=Transfer` carrying them (section 12.4.1 p.12-376, MUST) — and sources the `TagMatch` a `TagOp=Match` write owes, with its TgtID from **ReturnNID** (Table 3-1 p.3-153), and performs the match, reporting section 12.11.1's (p.12-386) accurate verdict. The HN-F holds one Allocation Tag per 16 bytes, a valid bit each and a Dirty bit per L3 line (`hnf_data_sram.sv`, `hnf_data_buffer.sv`); answers a `Transfer`/`Fetch` read `Transfer`, or `Update` with a response that passes Dirty (section 12.4.1 p.12-376), fetching the tags from memory where a snoop or the L3 returned data without them (sections 12.1, 12.9.1); writes Dirty tags back where a line leaves without passing them (section 12.3 p.12-374); uses `SnpCleanInvalid` for a `WriteUniqueFull`/`MakeUnique` that does not `Update` the tags (section 4.4.2 p.4-196, MUST); forwards `TagOp=Match` to its Subordinate with the TagGroupID and `ReturnNID` = Requester (sections 2.5.3, 12.10) and performs the match itself for an Atomic it executes (section 12.7); never uses a forwarding snoop for a read that requests tags (section 12.9.2). `BROADCASTMTE` is the asserted tie-off `HNF_BROADCASTMTE_PARAM` (section 16.2.6 p.16-476). Stash snoops follow section 12.9.3 (p.12-384): a `WriteUniqueFullStash` that does not `Update` the tags stashes with `SnpUniqueStash`, and a Data Pull returns the tags the Home holds. A merged `WriteUniquePtl` or an executed Atomic with `TagOp=Match` fetches the tags it lacks before matching — [#167](https://github.com/10x-Engineers/CHI-OpenNoC/issues/167) |
