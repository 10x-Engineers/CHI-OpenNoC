...
// per tag. TAGV packs the line's tags with a valid bit each -- Sec 12.3 (p.12-374,
// MUST) "Tags can be Valid only when data is Valid", so they live on the line's own
// index and way strobes and are evicted with it.
`define CACHE_TAG_WIDTH                    (chie_pkg::TAG_WIDTH*2)
`define CACHE_TV_WIDTH                     (chie_pkg::TU_WIDTH*2)
`define CACHE_TAGV_WIDTH                   (`CACHE_TAG_WIDTH + `CACHE_TV_WIDTH)
