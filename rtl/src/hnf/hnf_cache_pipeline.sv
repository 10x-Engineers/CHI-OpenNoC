...
module hnf_cache_pipeline `HNF_PARAM
    ...
    always_ff @(posedge clk or posedge rst)begin
        if (rst == 1'b1)begin
            ...
            pipe_sf_other_valid_mask_sx3_q[`RNF_NUM*2-1:0] <= ~({`RNF_NUM{2'b10}} | (({`RNF_NUM*2{pipe_rnfid_found_sx2}} & {{(`RNF_NUM*2-2){1'b0}}, 2'b01}) << (2*pipe_physical_nodeid_sx2[NID_WIDTH-1:0])));//01010101010101010101010101010100
            pipe_sf_other_share_mask_sx3_q[`RNF_NUM*2-1:0] <= ~({`RNF_NUM{2'b01}} | (({`RNF_NUM*2{pipe_rnfid_found_sx2}} & {{(`RNF_NUM*2-2){1'b0}}, 2'b10}) << (2*pipe_physical_nodeid_sx2[NID_WIDTH-1:0])));//10101010101010101010101010101000
            pipe_sf_self_valid_mask_sx3_q[`RNF_NUM*2-1:0]  <= {`RNF_NUM{2'b00}} | (({`RNF_NUM*2{pipe_rnfid_found_sx2}} & {{(`RNF_NUM*2-2){1'b0}}, 2'b01}) << (2*pipe_physical_nodeid_sx2[NID_WIDTH-1:0]));//00000000000000000000000000000001
            pipe_sf_self_share_mask_sx3_q[`RNF_NUM*2-1:0]  <= {`RNF_NUM{2'b00}} | (({`RNF_NUM*2{pipe_rnfid_found_sx2}} & {{(`RNF_NUM*2-2){1'b0}}, 2'b10}) << (2*pipe_physical_nodeid_sx2[NID_WIDTH-1:0]));//00000000000000000000000000000010
            pipe_sf_self_mask_sx3_q[`RNF_NUM*2-1:0]        <= {`RNF_NUM{2'b00}} | (({`RNF_NUM*2{pipe_rnfid_found_sx2}} & {{(`RNF_NUM*2-2){1'b0}}, 2'b11}) << (2*pipe_physical_nodeid_sx2[NID_WIDTH-1:0]));//00000000000000000000000000000011
        end
    end
    ...
    always_comb begin
        ...
        for (int ii = 0; ii < `LOC_WAY_NUM; ii = ii + 1)
    ...
    always_comb begin
        ...
        for (int ii = 0; ii < `SF_WAY_NUM; ii = ii + 1)begin
    ...
    always_comb begin
        for (int ii = 0; ii < `LOC_WAY_NUM; ii = ii + 1)begin
    ...
    assign pipe_read_l3_nofill_sx4 = pipe_tag_match_sx4_q & (
               ...
               op_wuptl_sx4_q | op_wufull_sx4_q |
               // Dirty Read l3 data
               (pipe_tag_match_dirty_sx4_q & (op_dl_cu_sx4_q | op_cmo_cs_sx4_q | op_cmo_ci_sx4_q | op_dl_mu_sx4_q))
           );
    ...
    always_ff @(posedge clk or posedge rst)begin
        if (rst == 1'b1)begin
            pipe_dbf_rd_idx_sx6_q <= {`MSHR_ENTRIES_WIDTH{1'b0}};
        end
        else if (pipe_req_valid_sx[SX5] == 1'b1)begin
