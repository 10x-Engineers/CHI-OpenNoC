...
module hnf_mshr_ctl `HNF_PARAM
    ...
    input  wire [`MSHR_ENTRIES_NUM-1:0]        dbf_mshr_tags_full_sx,
    input  wire [`MSHR_ENTRIES_NUM-1:0]        dbf_mshr_tags_any_sx,
    input  wire [`MSHR_ENTRIES_NUM-1:0]        dbf_mshr_tags_dirty_sx,
    input  wire                                dbf_mshr_tags_full_s0,
    ...
    // The line of zeros the Home sources for the two transactions that get their
    ...
    output logic [1:0]                         mshr_dbf_rd_dn_tagop_sx1,
    output logic [1:0]                         mshr_txreq_tagop_sx1,
    output wire [7:0]                          mshr_txreq_taggroupid_sx1,
    ...
    output logic [`HNF_PKTS-1:0]               mshr_dbf_home_fill_pe_sx1_q,
    ...
    logic [`MSHR_ENTRIES_NUM-1:0]        mshr_wrzero_dat_pend_sx_q;
    wire  [`MSHR_ENTRIES_NUM-1:0]        mshr_wrzero_dat_taken_sx;
    logic                                mshr_wrzero_inject_sx;
    logic [`MSHR_ENTRIES_WIDTH-1:0]      mshr_wrzero_inject_idx_sx;
    // The entry owes the Subordinate a System-cache eviction write; held until
    // retire, unlike abf_internal_evict_addr_valid_sx_q, which the hazard CAM
    ...
    // SS2.10.4 (p.2-136): packets of a transfer counted against its Size and Data_Width.
    logic [opennoc_hnf_pkg::HNF_PKT_CNT_W-1:0] mshr_snpdat_pkts_s1_q[0:`MSHR_ENTRIES_NUM-1];
    ...
    logic [`HNF_PKTS-1:0]                mshr_snp_getid_s1_q[0:`MSHR_ENTRIES_NUM-1];
    logic [`MSHR_ENTRIES_NUM-1:0]        mshr_dct_s1_q;
    logic [`MSHR_ENTRIES_NUM-1:0]        mshr_dat_entry_vec_s1_q;
    logic [opennoc_hnf_pkg::HNF_PKT_CNT_W-1:0] mshr_dat_rn_pkts_s1_q[0:`MSHR_ENTRIES_NUM-1];
    logic [opennoc_hnf_pkg::HNF_PKT_CNT_W-1:0] mshr_dat_mem_pkts_s1_q[0:`MSHR_ENTRIES_NUM-1];
    ...
    wire [`MSHR_ENTRIES_NUM-1:0]         mshr_dbf_home_fill_entry_sx1;
    ...
    wire [`MSHR_ENTRIES_NUM-1:0]         mshr_snp_getid_set_s1;
    wire [`HNF_PKTS-1:0]                 mshr_snpdat_pkt_s0;
    ...
    wire [`MSHR_ENTRIES_NUM-1:0]   mshr_mem_cmo_busy_set_sx;
    wire [`MSHR_ENTRIES_NUM-1:0]   mshr_mem_cmo_busy_clr_sx;
        ...
        for(entry=0;entry<`MSHR_ENTRIES_NUM;entry=entry+1) begin
            ...
            always_ff @(posedge clk)begin : mshr_stash_sep_s1_q_timing_logic
                if(mshr_req_clr_sx1[entry] == 1'b1)
                    ...
                    mshr_stash_sep_s1_q[entry] <= li_mshr_rxreq_stash_sep_s0;
                else
                    ;
            end

            always_ff @(posedge clk)begin : mshr_wrzero_s1_q_timing_logic
                if(mshr_req_clr_sx1[entry] == 1'b1)
                    mshr_wrzero_s1_q[entry] <= 1'b0;
                else if(mshr_req_set_s0[entry] == 1'b1)
                    mshr_wrzero_s1_q[entry] <= li_mshr_rxreq_wrzero_s0;
                else
                    ;
            end

            always_ff @(posedge clk)begin : mshr_cw_s1_q_timing_logic
                if(mshr_req_clr_sx1[entry] == 1'b1)
                    mshr_cw_s1_q[entry] <= 1'b0;
                else if(mshr_req_set_s0[entry] == 1'b1)
                    mshr_cw_s1_q[entry] <= li_mshr_rxreq_cw_s0;
                else
                    ;
            end
            ...
            always_ff @(posedge clk)begin : mshr_l3_alloc_s1_q_timing_logic
                if(mshr_req_clr_sx1[entry] == 1'b1)
                    ...
                    mshr_l3_alloc_s1_q[entry] <= li_mshr_rxreq_l3_alloc_s0;
                else
                    ;
            end
            ...
            // Sec 13.10.7 (p.13-418): the PGroupID occupies the request's LPID bits
            // and the response's DBID bits, and Sec 2.5.6 (p.2-89, MUST) has the
            // Completer "reflect back PGroupID in the Persist and CompPersist".
            always_ff @(posedge clk)begin : mshr_pgroupid_s1_q_timing_logic
                if(mshr_req_clr_sx1[entry] == 1'b1)
    ...
    generate
        for(entry=0;entry<`MSHR_ENTRIES_NUM;entry=entry+1) begin : mshr_snp_decode_comb_logic
            ...
            // The last packet of a Snoop response's 64-byte line.
            assign mshr_snpdat_gettwo_s0[entry]    = mshr_snpdat_entry_vec_s0[entry] &
                                                     (mshr_snpdat_pkts_s1_q[entry] == opennoc_hnf_pkg::HNF_PKT_CNT_W'(`HNF_PKTS - 1));
            ...
            assign mshr_snp_getid_set_s1[entry]    = mshr_snpdat_entry_vec_s0[entry];
            ...
            assign mshr_snp_get_64B_s1[entry]      = &mshr_snp_getid_s1_q[entry];
            // Every data packet arrived, AND every byte in them is valid. Sec 2.10.3
            ...
            assign mshr_wuf_memwr_s1[entry]        = (mshr_wuf_seq[entry] & mshr_snp_getall_s1[entry] & (mshr_snpdat_entry_vec_s1_q[entry] | mshr_snprsp_entry_vec_s1_q[entry]));
    ...
    generate
        for (entry=0;entry<`MSHR_ENTRIES_NUM;entry=entry+1) begin : mshr_snpdat_pkts_s1_q_timing_logic
            always_ff @(posedge clk or posedge rst)begin
                if (rst == 1'b1)
                    mshr_snpdat_pkts_s1_q[entry] <= '0;
                else if (mshr_snpdat_gettwo_s0[entry])
                    mshr_snpdat_pkts_s1_q[entry] <= '0;
                else if (mshr_snpdat_entry_vec_s0[entry])
                    mshr_snpdat_pkts_s1_q[entry] <= mshr_snpdat_pkts_s1_q[entry] + 1'b1;
                else
                    ;
            end
        end
    ...
    generate
        for (entry=0;entry<`MSHR_ENTRIES_NUM;entry=entry+1) begin : mshr_snp_getid_s1_q_timing_logic
            always_ff @(posedge clk or posedge rst)begin
                if(rst == 1'b1)
                    mshr_snp_getid_s1_q[entry] <= '0;
                else if(mshr_snp_getid_clr_s1[entry])
                    mshr_snp_getid_s1_q[entry] <= '0;
                else if(mshr_snp_getid_set_s1[entry])
                    mshr_snp_getid_s1_q[entry] <= mshr_snp_getid_s1_q[entry] | mshr_snpdat_pkt_s0;
                else
                    ;
            end
        end
    ...
    // The write-data event a Write Zero raises for itself carries no flit, so it
    // names its own entry and delivers the whole line at once.
    assign mshr_dat_entry_s0       = li_mshr_rxdat_valid_s0 ? li_mshr_rxdat_txnid_s0
                                                            : 12'(mshr_wrzero_inject_idx_sx);
    ...
    assign mshr_set_stop_cb_s0     = mshr_rn_dat_s0 & ((mshr_rn_dat_get_i_sc_s0 & (mshr_wb_s1_q[mshr_dat_entry_idx_s0] | mshr_we_s1_q[mshr_dat_entry_idx_s0])) |
            ...
            (mshr_wb_s1_q[mshr_dat_entry_idx_s0] & ~mshr_l3_alloc_s1_q[mshr_dat_entry_idx_s0] & mshr_rn_dat_get_uc_s0));
        ...
        for(entry=0;entry<`MSHR_ENTRIES_NUM;entry=entry+1) begin : mshr_dat_decode_comb_logic
            ...
            assign mshr_new_dat_l3fill_s1[entry] = (mshr_dat_new_get_s1_q[entry] & (mshr_dat_entry_vec_s1_q[entry]) & (((mshr_wb_s1_q[entry]) & mshr_l3_alloc_s1_q[entry]) |
                   ...
                   (mshr_dat_new_get_s1_q[entry] & (mshr_dat_entry_vec_s1_q[entry] | mshr_l3_entry_vec_sx8_q[entry] |
                    ...
                    (mshr_tagop_update[entry] | mshr_snp_getall_s1[entry] | mshr_neednosnp_sx8_q[entry]));
        ...
        for(entry=0;entry<`MSHR_ENTRIES_NUM;entry=entry+1) begin
            ...
            always_ff @(posedge clk or posedge rst)begin : mshr_dat_stop_cb_s1_q_timing_logic
                if(rst == 1'b1)
                    ...
                    mshr_dat_stop_cb_s1_q[entry] <= 1'b0;
                else if(mshr_set_stop_cb_s0 && mshr_dat_entry_vec_s0[entry] && mshr_rn_dat_getall_s0)
                    mshr_dat_stop_cb_s1_q[entry] <= 1'b1;
                else
                    ;
            end

            always_ff @(posedge clk or posedge rst) begin: mshr_dat_mem_pkts_s1_q_timing_logic
                if(rst == 1'b1)
                    mshr_dat_mem_pkts_s1_q[entry] <= '0;
                else if(mshr_can_retire_entry_sx1[entry])
                    mshr_dat_mem_pkts_s1_q[entry] <= '0;
                else if(mshr_dat_entry_vec_s0[entry] && mshr_mem_dat_s0 &&
                        (mshr_dat_mem_pkts_s1_q[entry] != opennoc_hnf_pkg::HNF_PKT_CNT_W'(`HNF_PKTS)))
                    mshr_dat_mem_pkts_s1_q[entry] <= mshr_dat_mem_pkts_s1_q[entry] + 1'b1;
                else
                    ;
            end
            ...
            always_ff @(posedge clk or posedge rst) begin: mshr_dat_old_get_s1_q_timing_logic
                if(rst == 1'b1)
                    ...
                    mshr_dat_old_get_s1_q[entry] <= (int'(mshr_dat_mem_pkts_s1_q[entry]) + 1 == `HNF_PKTS);
                ...
                // the accumulated byte enables, not of the last packet arriving.
            ...
            always_ff @(posedge clk or posedge rst) begin: mshr_dat_rn_pkts_s1_q_timing_logic
                if(rst == 1'b1)
                    mshr_dat_rn_pkts_s1_q[entry] <= '0;
                else if(mshr_can_retire_entry_sx1[entry])
                    mshr_dat_rn_pkts_s1_q[entry] <= '0;
                else if(mshr_dat_entry_vec_s0[entry] && mshr_rn_dat_s0 &&
                        (mshr_dat_rn_pkts_s1_q[entry] != opennoc_hnf_pkg::HNF_PKT_CNT_W'(`HNF_PKTS)))
                    mshr_dat_rn_pkts_s1_q[entry] <= mshr_dat_rn_pkts_s1_q[entry] + 1'b1;
                else
                    ;
            end
            ...
            always_ff @(posedge clk or posedge rst) begin: mshr_cb_wr_mem_s1_q_timing_logic
                if(rst == 1'b1)
                    ...
                    mshr_cb_wr_mem_s1_q[entry] <= mshr_rn_dat_getall_s0 & mshr_rn_dat_get_d_s0 & ((mshr_wb_s1_q[entry] & (~mshr_l3_alloc_s1_q[entry])) | (mshr_wc_s1_q[entry]));
                else
                    ;
            end
    ...
    generate
        for(entry=0;entry<`MSHR_ENTRIES_NUM;entry=entry+1) begin : mshr_rsp_entry_vec_s0_comb_logic
            ...
            assign mshr_retosrc_entry_vec_sx8[entry] = (mshr_ro_s1_q[entry] & ~l3_snpdirect_sx7_q) ||
                   ...
                   (mshr_rc_s1_q[entry]) ||
                   (mshr_wup_s1_q[entry] & mshr_l3_alloc_s1_q[entry] & ~l3_hit_sx7_q);
        end
    endgenerate
    ...
    generate
        for(entry=0;entry<`MSHR_ENTRIES_NUM;entry=entry+1) begin
            ...
            always_ff @(posedge clk or posedge rst)begin : mshr_dmt_sx8_q_timing_logic
                ...
                // The tags a Home-held line lacks are fetched for the Home, not the Requester.
    ...
    generate
        for(entry=0;
            ...
            assign mshr_l3_memwr_sx7[entry]     = mshr_l3_entry_vec_sx7[entry] & ((l3_hit_d_sx7_q & (l3_opcode_sx7_q == chie_pkg::REQ_CLEANSHARED | l3_opcode_sx7_q == chie_pkg::REQ_CLEANINVALID | l3_opcode_sx7_q == chie_pkg::REQ_CLEANUNIQUE | (!l3_sfhit_sx7_q & l3_opcode_sx7_q == chie_pkg::REQ_READCLEAN))) |
                    ((l3_opcode_sx7_q == chie_pkg::REQ_WRITEUNIQUEPTL) & ~mshr_l3_alloc_s1_q[entry] & ~l3_sfhit_sx7_q & l3_rd_busy_s2_q[entry]) |
            ...
            assign mshr_l3_tagwb_sx7[entry]     = mshr_l3_entry_vec_sx7[entry] & l3_hit_sx7_q & l3_hit_d_sx7_q & l3_tags_dirty_sx7_q & l3_rd_busy_s2_q[entry] &
                   ((((l3_opcode_sx7_q == chie_pkg::REQ_READUNIQUE) |
                    ...
                    ((l3_opcode_sx7_q == chie_pkg::REQ_MAKEUNIQUE) & ~mshr_tagop_update[entry]));
    ...
    generate
        for(entry=0;
            ...
            assign mshr_rn_data_busy_set_sx[entry]   = (mshr_atomicrd_s1_q[entry] & mshr_can_alloc_entry_s1_q[entry]) ||
                   ...
                   (mshr_dat_to_rn_s1[entry]) || (mshr_l3dat_rn_sx7[entry] & ~mshr_l3_tagfetch_sx7[entry]) ||
            ...
            assign mshr_txdat_rn_rdy_set_sx[entry]   = (mshr_atm_dat_owed_sx[entry]) ||
                   ...
                   (mshr_l3dat_rn_sx7[entry] & mshr_neednosnp_sx7[entry] & ~mshr_l3_tagfetch_sx7[entry]) ||
    ...
    generate
        for(entry=0;
            ...
            always_ff @(posedge clk or posedge rst)begin : mshr_mem_cmo_busy_sx_q_timing_logic
                if(rst == 1'b1)
                    mshr_mem_cmo_busy_sx_q[entry] <= 1'b0;
                else if(mshr_can_retire_entry_sx1[entry])
                    mshr_mem_cmo_busy_sx_q[entry] <= 1'b0;
                else if(mshr_mem_cmo_busy_set_sx[entry])
                    mshr_mem_cmo_busy_sx_q[entry] <= 1'b1;
                else if(mshr_mem_cmo_busy_clr_sx[entry])
                    mshr_mem_cmo_busy_sx_q[entry] <= 1'b0;
                else
                    ;
            end

            always_ff @(posedge clk or posedge rst)begin : mshr_mem_cmo_rdy_sx_q_timing_logic
                if(rst == 1'b1)
                    mshr_mem_cmo_rdy_sx_q[entry] <= 1'b0;
                else if(mshr_can_retire_entry_sx1[entry])
                    mshr_mem_cmo_rdy_sx_q[entry] <= 1'b0;
                else if(mshr_mem_cmo_rdy_set_sx[entry])
                    mshr_mem_cmo_rdy_sx_q[entry] <= 1'b1;
                else if(mshr_mem_cmo_rdy_clr_sx[entry])
                    mshr_mem_cmo_rdy_sx_q[entry] <= 1'b0;
                else
                    ;
            end
    ...
    generate
        for (entry=0;
            ...
            // point that holds in every completion shape -- including the
            ...
            always_ff @(posedge clk or posedge rst)begin : mshr_cw_owed_sx_q_timing_logic
                if(rst == 1'b1)
            ...
            always_ff @(posedge clk or posedge rst)begin : mshr_cw_rdy_sx_q_timing_logic
                if(rst == 1'b1)
    ...
    assign mshr_txreq_evict_wr_sx1    = mshr_evict_pending_sx_q[mshr_txreq_entry_idx_sx1] & ~mshr_txreq_is_rd_sx1 & ~mshr_txreq_is_cmo_sx1;
    ...
    assign mshr_txreq_icn_wr_sx1      = ~mshr_txreq_is_rd_sx1 & ~mshr_txreq_is_cmo_sx1
    ...
    // Sec 11.3.4 (p.11-366, MUST) binds the MPAM of a request to the Subordinate only
    // "where the request is generated by a request to the Home". A snoop-filter evict,
    // an SLC eviction and its write-back answer to none, so they have no label to
    // propagate -- and Sec 11.3 (p.11-365, MUST) then fixes what they must carry: "when
    // a sender does not want to use MPAM for a request, the MPAM values must be set to
    // default settings", Table 11-5 (p.11-366). Same three-term condition the MemAttr
    // and RSVDC above already take, for the same reason.
    assign mshr_txreq_mpam_sx1        = (mshr_seq_s1_q[mshr_txreq_entry_idx_sx1]
                                       | mshr_txreq_evict_wr_sx1
                                       | mshr_txreq_icn_wr_sx1)
                                      ? chie_pkg::mpam_default(mshr_txreq_ns_sx1)
                                      : mshr_mpam_s1_q[mshr_txreq_entry_idx_sx1];
    ...
    // Table 4-38 (SS4.7.2 p.4-218): a StashOnceSep* completes with CompStashDone.
    ...
    // Sec 2.3.3 (p.2-69) lets the Home wait for the write data before returning an
    // Atomic's CompData, and that data lands in the same buffer entry the fill
    ...
    generate
        for(entry=0;entry<`MSHR_ENTRIES_NUM;entry=entry+1) begin : mshr_home_fill_pend_logic
            assign mshr_home_fill_req_sx[entry] = (mshr_errrd_s1_q[entry] &
                   (mshr_errwrdat_s1_q[entry] ? (mshr_dat_entry_vec_s1_q[entry] & mshr_dat_new_get_s1_q[entry])
                                              : mshr_can_alloc_entry_s1_q[entry]))
                 | (mshr_wrzero_s1_q[entry] & mshr_can_alloc_entry_s1_q[entry]);
            always_ff @(posedge clk or posedge rst)begin : mshr_home_fill_pend_sx_q_timing_logic
                if(rst == 1'b1)
                    mshr_home_fill_pend_sx_q[entry] <= 1'b0;
                else if(mshr_home_fill_req_sx[entry])
                    mshr_home_fill_pend_sx_q[entry] <= 1'b1;
                else if(mshr_dbf_home_fill_valid_sx1_q && mshr_dbf_home_fill_idx_sx1_q == entry[`MSHR_ENTRIES_WIDTH-1:0])
                    mshr_home_fill_pend_sx_q[entry] <= 1'b0;
                else
                    ;
            end
    ...
    // write-data event every downstream arm gates on has no RXDAT flit to come
    // from. It is raised here instead, one entry per cycle and never in a cycle a
    // real flit is using the decode below -- and only once the Home-sourced line
    // is actually in the buffer, so nothing reads the entry before it is written.
    generate
        for(entry=0;entry<`MSHR_ENTRIES_NUM;entry=entry+1) begin : mshr_wrzero_dat_pend_logic
            assign mshr_wrzero_dat_taken_sx[entry] = mshr_wrzero_inject_sx &
                   (mshr_wrzero_inject_idx_sx == entry[`MSHR_ENTRIES_WIDTH-1:0]);
            always_ff @(posedge clk or posedge rst)begin : mshr_wrzero_dat_pend_sx_q_timing_logic
                if(rst == 1'b1)
                    mshr_wrzero_dat_pend_sx_q[entry] <= 1'b0;
                else if(mshr_dbf_home_fill_valid_sx1_q & mshr_wrzero_s1_q[entry] &
                        (mshr_dbf_home_fill_idx_sx1_q == entry[`MSHR_ENTRIES_WIDTH-1:0]))
                    mshr_wrzero_dat_pend_sx_q[entry] <= 1'b1;
                else if(mshr_wrzero_dat_taken_sx[entry] | mshr_can_retire_entry_sx1[entry])
                    mshr_wrzero_dat_pend_sx_q[entry] <= 1'b0;
                else
                    ;
            end
    ...
    always_comb begin : mshr_wrzero_inject_pick
        mshr_wrzero_inject_sx     = 1'b0;
        mshr_wrzero_inject_idx_sx = {`MSHR_ENTRIES_WIDTH{1'b0}};
        for(int i=`MSHR_ENTRIES_NUM-1;i>=0;i=i-1) begin
            if(mshr_wrzero_dat_pend_sx_q[i] && !li_mshr_rxdat_valid_s0) begin
                mshr_wrzero_inject_sx     = 1'b1;
                mshr_wrzero_inject_idx_sx = i[`MSHR_ENTRIES_WIDTH-1:0];
            end
        end
    end

    always_comb begin : mshr_home_fill_pick
        mshr_home_fill_pick_valid_sx = 1'b0;
        mshr_home_fill_pick_idx_sx   = {`MSHR_ENTRIES_WIDTH{1'b0}};
        for(int i=`MSHR_ENTRIES_NUM-1;i>=0;i=i-1) begin
            if(mshr_home_fill_pend_sx_q[i] && !(mshr_dbf_home_fill_valid_sx1_q && mshr_dbf_home_fill_idx_sx1_q == i[`MSHR_ENTRIES_WIDTH-1:0])) begin
                mshr_home_fill_pick_valid_sx = 1'b1;
                mshr_home_fill_pick_idx_sx   = i[`MSHR_ENTRIES_WIDTH-1:0];
            end
        end
    end
    ...
    // from one would otherwise emit every packet of the line for a sub-line transfer.
    //
    ...
    always_ff @(posedge clk or posedge rst)begin : mshr_dbf_home_fill_timing_logic
        if(rst == 1'b1) begin
            mshr_dbf_home_fill_valid_sx1_q <= 1'b0;
            mshr_dbf_home_fill_idx_sx1_q   <= {`MSHR_ENTRIES_WIDTH{1'b0}};
            mshr_dbf_home_fill_be_sx1_q    <= '0;
            mshr_dbf_home_fill_pe_sx1_q    <= '0;
        end
        else begin
    ...
    generate
        for(entry=0;
            ...
            assign mshr_dbf_home_fill_entry_sx1[entry] = (mshr_dbf_home_fill_idx_sx1_q == entry) & mshr_dbf_home_fill_valid_sx1_q;
        end
    endgenerate
