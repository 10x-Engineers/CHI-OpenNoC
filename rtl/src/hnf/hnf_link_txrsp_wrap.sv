...
module hnf_link_txrsp_wrap `HNF_PARAM
    ...
    // link left RUN still goes out, and the credit it returns is the same credit.
    // Only STOP and ACTIVATE are the states Table 14-3 bars a flit in.
    ...
    // Table 14-2 DEACTIVATE (p.14-450, MUST): "The Transmitter must return credits
    // using Protocol flits or L-Credit return flits" -- alternative uses of the
    // SAME credit, so a return sent while a response is still owed spends the one
    // that response needs, and the peer has stopped granting. Stated over the
    // request, not over the flit being driven: the flop below gives the return the
    // higher priority. snf_txrsp.sv writes the same condition.
