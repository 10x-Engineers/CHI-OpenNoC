...
module hnf_link_rxreq_parse `HNF_PARAM
    ...
    output wire                                li_mshr_rxreq_stash_sep_s0,
    ...
    output wire [1:0]                          li_mshr_rxreq_tagop_s0,
    ...
    // the Opcode field." It carries no request, only the L-Credit it returns, so the
    ...
    assign li_mshr_rxreq_tagop_s0      = rxreq_flit_valid_s0? rxreqflit.tagop     :'0;
    ...
    assign li_mshr_rxreq_rsvdc_s0      = rxreq_flit_valid_s0? chie_pkg::req_rsvdc_of(rxreqflit) :'0;
    ...
    // Table 14-2 (p.14-450, MUST): a Receiver must not send credits outside RUN. The
    // grant is decided on rxcrd_en but the valid is registered, so it is held until RUN
    // resumes rather than leaving a cycle late; the pool is already debited for it.
    assign rxreq_lcrdv = rxreqcrdv_s1_q & rxcrd_en;
    //-----------------------------------------------------------------------------
    // DISPLAY INFO
    //-----------------------------------------------------------------------------
`ifdef DISPLAY_INFO
